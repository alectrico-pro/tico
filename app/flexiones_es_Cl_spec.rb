...
RSpec.describe :flexiones_es_Cl do
  ...
  describe 'Inflexiones generales para es-MX' do
      ...
      context contexto do
        sp.each do |singular_plural|
          ...
          it "el singular de #{plural} es #{singular}" do
            expect(plural.singularize(:"es-MX")).to eq singular
          end
          
          it "el plural de #{singular} es #{plural}" do
            expect(singular.pluralize(:"es-MX")).to eq plural
          end
